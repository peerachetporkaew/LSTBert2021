...
def load_dictionaries(path):
    path = Path.cwd() / path
    pos_dict = Dictionary().load(open(path / "./lst20-pos/pos.txt","r",encoding="utf-8"))
    pos_dict.add_symbol("<s>")
    pos_dict.add_symbol("</s>")
    pos_dict.finalize()
    print(pos_dict.symbols)

    ne_dict = Dictionary().load(open(path / "./lst20-ne/ne.txt","r",encoding="utf-8"))
    ne_dict.add_symbol("<s>")
    ne_dict.add_symbol("</s>")
    ne_dict.finalize()
    print(ne_dict.symbols)

    sent_dict = Dictionary().load(open(path / "./lst20-sent/sent.txt","r",encoding="utf-8"))
